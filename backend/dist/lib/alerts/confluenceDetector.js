...
exports.confluenceDetector = exports.ConfluenceDetector = exports.SetupType = exports.AlertSeverity = void 0;
var AlertSeverity;
...
(function (SetupType) {
    ...
    SetupType["CAPITULATION_BOTTOM"] = "CAPITULATION_BOTTOM";
    SetupType["CAPITULATION_TOP"] = "CAPITULATION_TOP";
    SetupType["BULLISH_DIVERGENCE"] = "BULLISH_DIVERGENCE";
    SetupType["BEARISH_DIVERGENCE"] = "BEARISH_DIVERGENCE";
})(SetupType || (exports.SetupType = SetupType = {}));
class ConfluenceDetector {
    constructor() {
        this.timeSeries = new Map();
        this.fundingRateStats = new Map();
        this.lastAlertTime = new Map();
        this.ALERT_COOLDOWN = 8 * 60 * 60 * 1000; // 8 hours between alerts per symbol (max 3 per day)
        // Time windows (in milliseconds)
        this.ONE_HOUR = 60 * 60 * 1000;
        this.EIGHT_HOURS = 8 * 60 * 60 * 1000;
        this.TWENTY_FOUR_HOURS = 24 * 60 * 60 * 1000;
        this.MAX_HISTORY = 48 * 60 * 60 * 1000; // Keep 48 hours of data
    }
    ...
    detectPatterns(currentData) {
        ...
        const highLiquidityCoins = currentData
            .filter(d => d.quoteVolume > 50000000) // Min $50M 24h volume (stricter)
    ...
    updateTimeSeries(data, now) {
        ...
        this.timeSeries.set(data.symbol, series.filter(d => d.timestamp > cutoff));
    }
    // Helper: Check if we have enough history
    hasEnoughHistory(symbol, now) {
    ...
    calculateOIChange(symbol, now, windowMs) {
        const series = this.timeSeries.get(symbol);
    ...
    calculateVDelta(symbol, now, windowMs) {
        const series = this.timeSeries.get(symbol);
        if (!series || series.length < 2)
            return 0;
        const cutoff = now - windowMs;
    ...
    calculatePriceChange(symbol, now, windowMs) {
        const series = this.timeSeries.get(symbol);
        if (!series || series.length < 2)
            return 0;
        const cutoff = now - windowMs;
    ...
    calculateFundingZScore(symbol, currentFunding) {
...
exports.ConfluenceDetector = ConfluenceDetector;
// Export singleton
exports.confluenceDetector = new ConfluenceDetector();
