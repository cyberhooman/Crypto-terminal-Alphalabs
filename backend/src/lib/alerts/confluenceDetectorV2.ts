...
export class ConfluenceDetectorV2 {
  private timeSeries: Map<string, TimeSeriesData[]> = new Map();
  private lastAlertTime: Map<string, number> = new Map();

  // Configuration
  private readonly ALERT_COOLDOWN = 4 * 60 * 60 * 1000; // 4 hours (6 signals/day max per symbol)
  private readonly LOOKBACK_WINDOW = 30 * 24 * 60 * 60 * 1000; // 30 days for percentiles
  private readonly MIN_OI_VALUE = 10_000_000; // $10M minimum OI
  private readonly MIN_VOLUME = 50_000_000; // $50M 24h volume
  private readonly SCORE_THRESHOLD = 75; // Minimum 75/100 to alert
  ...
  private detectShortSqueeze(
    data: MarketData,
    fundingPercentile: number,
    oiStats: PercentileStats,
    now: number
  ...
  private calculateFundingPercentile(symbol: string, currentFunding: number): number {
    const history = this.timeSeries.get(symbol) || [];
    if (history.length < 7 * 24) return 50; // Need 7 days minimum
  ...
  private calculateOIStats(symbol: string, now: number): PercentileStats {
    const history = this.timeSeries.get(symbol) || [];
  ...
  private calculateOIChange(symbol: string, now: number, window: number): number {
    const history = this.timeSeries.get(symbol) || [];
    const past = history.find(h => Math.abs(h.timestamp - (now - window)) < 10 * 60 * 1000);
    const current = history[history.length - 1];

    if (!past || !current || past.oi === 0) return 0;
    return ((current.oi - past.oi) / past.oi) * 100;
  }

  // Calculate VDelta over time window
  private calculateVDelta(symbol: string, now: number, window: number): number {
    const history = this.timeSeries.get(symbol) || [];
  ...
  private calculatePriceChange(symbol: string, now: number, window: number): number {
    const history = this.timeSeries.get(symbol) || [];
    const past = history.find(h => Math.abs(h.timestamp - (now - window)) < 10 * 60 * 1000);
    const current = history[history.length - 1];

    if (!past || !current || past.price === 0) return 0;
    return ((current.price - past.price) / past.price) * 100;
  }

  // Calculate funding rate momentum (derivative)
  private calculateFundingMomentum(symbol: string, now: number): number {
    const history = this.timeSeries.get(symbol) || [];
  ...
  private updateTimeSeries(data: MarketData, now: number) {
  ...
  private hasMinimumHistory(symbol: string, now: number): boolean {
    const history = this.timeSeries.get(symbol) || [];
